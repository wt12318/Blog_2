...
在`Scikit-Learn`中可以指定超参数`C`来决定对间隔中的点“惩罚”强度(见[SVM理论部分](https://wutaoblog.com.cn/p/svm_theory/)软间隔最大化的优化问题);如果C比较小,那么惩罚比较小,所以在间隔中的点也比较多,相反,如果C比较大,在间隔中的点就比较少:

![](https://picgo-wutao.oss-cn-shanghai.aliyuncs.com/img/image-20210403220433035.png)
因此,在SVM模型过拟合的时候可以考虑减少C

下面是一个在鸢尾花数据集上训练SVM模型的例子:
...
通过核技巧将线性不可分的数据映射到高维空间，使其更有可能线性可分，通常使用的核函数有：多项式核函数和高斯核函数，可以通过
SVC 来使用各种核函数，核函数的表达式和参数如下表：
